...
class ProgressBar:
    ...
    def prog(self, i: int, max_iter: int, epoch: Union[int, str],
                     task_number: int, loss: float, train_acc, test_acc=0, num_params=None, num_neurons=None) -> None:
        """
        ...
        if i:  # not (i + 1) % 10 or (i + 1) == max_iter:
            ...
            print('\r[ {} ] Task {} | epoch {}: |{}| {} ep/h | loss: {} | train acc: {} % | test acc: {} % | params {} | neurons {}|'.format(
                datetime.now().strftime("%m-%d | %H:%M"),
                task_number + 1 if isinstance(task_number, int) else task_number,
                epoch,
                progress_bar,
